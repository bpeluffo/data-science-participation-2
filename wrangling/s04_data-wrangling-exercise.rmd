...
arrange(select( year, lifeExp, country), year, lifeExp)
...
gapminder %>%
  mutate(grossGDP = gdpPercap * pop),
        gdpBill = round (grossGDP/ 1000000000)


gapminder %>%
  mutate(grossGDP = gdpPercap * pop,
  gdpBill = round (grossGDP/ 1000000000) %>% round (2))
...
Imagine we learned that all of the Bangladesh life expectancy values are unreliable.
